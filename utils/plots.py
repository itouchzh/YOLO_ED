    ...
    if 'Detect' not in module_type:
        ...
        if height > 1 and width > 1:
            ...
            for i in range(n):
                ...
                ax.imshow(blocks[i].squeeze())  # cmap='gray'
                ax.axis('off')

                f = save_dir / f"stage{stage}_{module_type.split('.')[-1]}_feature_{i}.png"  # filename
                LOGGER.info(f'Saving {f}... ({i + 1}/{channels})')
                plt.savefig(f, dpi=300, bbox_inches='tight', pad_inches=0)
                plt.close()

                np.save(str(f.with_suffix('.npy')), blocks[i].squeeze().cpu().numpy())  # npy save
